...
% p_{N-1}(x) = b(1)*(x-x_N)^3 + b(2)*(x-x_N)^2 + b(3)(x-x_N) + b(4)
b = coeffs(end,:);
...
pNm1_deriv_xN = 3*b(1)*(xdata(end) - xdata(end-1))^2 + ...
       2*b(2)*(xdata(end)-xdata(end-1)) + b(3);
...
for i = 2:N
    b(i-1) = 3*(1/h(i-1))^2*(ydata(i)- ydata(i-1)) + ...
               3*(1/h(i))^2*(ydata(i+1) - ydata(i));
end
